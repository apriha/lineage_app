...
class SharedDnaGenesForm(forms.ModelForm):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        self.fields['individual1'].queryset = user.individuals.all().order_by('created_at')
        self.fields['individual2'].queryset = Individual.objects.none()

        if 'individual1' in self.data:
            try:
                self.fields['individual2'].queryset = user.individuals.all().exclude(
                    pk=self.data.get('individual1')).order_by('created_at')
...
class DiscordantSnpsForm(forms.ModelForm):
    ...
    def __init__(self, user, *args, **kwargs):
        ...
        if 'individual2' in self.data:
            try:
                self.fields['individual3'].queryset = user.individuals.all().exclude(Q(
                    pk=self.data.get('individual1')) | Q(pk=self.data.get('individual2'))).order_by('created_at')
            except (ValueError, TypeError):
                pass  # invalid input from the client; ignore and fallback to empty queryset
