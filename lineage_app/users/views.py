...
def setup(request):
    ...
    if not request.user.setup_started:
        ...
        request.user.save()
        setup_user.apply_async((request.user.id,), task_id=str(request.user.setup_task_id))

    return render(request, 'users/setup.html', context={'task_id': str(request.user.setup_task_id)})
