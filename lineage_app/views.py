        ...
        try:
            ...
            if form.is_valid():
                ...
                transaction.on_commit(
                    lambda: setup_snps.apply_async(
                        (snps.id,), task_id=str(snps.setup_task_id)
                    )
                )
...
def edit_individual(request, uuid):
    ...
    return render(
        request,
        "pages/add_edit_individual.html",
        {
            "title": "Edit",
            "form": form,
...
def shared_dna_genes(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            if shared_dna_genes_calc_exists(form.cleaned_data):
                ...
                transaction.on_commit(
                    lambda: find_shared_dna_genes.apply_async(
                        (shared_dna_genes.id,),
    ...
    queryset = (
        request.user.shared_dna_genes.all().exclude(setup_complete=False).order_by("id")
    )
    ...
    return render(
        request,
        "pages/shared_dna_genes.html",
        {
            "form": form,
            "table": table,
            "finding_shared_dna_genes": request.user.shared_dna_genes.filter(
...
def shared_dna_genes_details(request, uuid):
    ...
    return render(
        request,
        "pages/shared_dna_genes_details.html",
        {
...
def shared_dna_one_chrom(request, uuid):
    ...
    return sendfile(
        request,
        shared_dna_genes.shared_dna_one_chrom_csv.path,
        attachment=True,
...
def shared_dna_two_chrom(request, uuid):
    ...
    return sendfile(
        request,
        shared_dna_genes.shared_dna_two_chrom_csv.path,
        attachment=True,
...
def shared_genes_one_chrom(request, uuid):
    ...
    return sendfile(
        request,
        shared_dna_genes.shared_genes_one_chrom_csv.path,
        attachment=True,
...
def shared_genes_two_chrom(request, uuid):
    ...
    return sendfile(
        request,
        shared_dna_genes.shared_genes_two_chrom_csv.path,
        attachment=True,
...
def discordant_snps(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            transaction.on_commit(
                lambda: find_discordant_snps.apply_async(
                    (discordant_snps.id,), task_id=str(discordant_snps.setup_task_id)
                )
            )
    ...
    queryset = (
        request.user.discordant_snps.all().exclude(setup_complete=False).order_by("id")
    )
    ...
    return render(
        request,
        "pages/discordant_snps.html",
        {
            "form": form,
            "table": table,
            "finding_discordant_snps": request.user.discordant_snps.filter(
                setup_complete=False
            ).count(),
        },
    )
...
def download_discordant_snps(request, uuid):
    ...
    return sendfile(
        request,
        discordant_snps.discordant_snps_csv.path,
        attachment=True,
...
def load_individual2_dropdown(request):
    ...
    if uuid != "":
        ...
        individuals = []
    return render(
        request, "pages/individual_dropdown_options.html", {"individuals": individuals}
    )
...
def load_individual3_dropdown(request):
    ...
    if uuid1 != "" and uuid2 != "":
        ...
        individuals = (
            request.user.individuals.all()
            .exclude(Q(pk=uuid1) | Q(pk=uuid2))
            .order_by("pk")
        )
    else:
        individuals = []
